  ...
  return (
    <Box>
      ...
      >
        ...
        <Flex flex={{ base: 1 }} justify={{ base: 'center', md: 'start' }}>
          <HStack as={RouterLink} to="/" spacing={2}>
            <Image src="/logo-small.svg" alt="Digital Wallet Logo" boxSize="30px" />
            <Text
      ...
      <Collapse in={isOpen} animateOpacity>
        <MobileNav isAuthenticated={isAuthenticated} logout={handleLogout} />
      </Collapse>
    </Box>
  );
}
...
const DesktopNav = ({ isAuthenticated }) => {
  ...
  // Only show certain nav items based on authentication status
  const filteredNavItems = isAuthenticated 
    ? NAV_ITEMS.filter(item => !item.authHidden)
...
const MobileNav = ({ isAuthenticated, logout }) => {
  // Only show certain nav items based on authentication status
  const filteredNavItems = isAuthenticated 
    ? NAV_ITEMS.filter(item => !item.authHidden)
    : NAV_ITEMS.filter(item => !item.requiresAuth);
  ...
  return (
    ...
    >
      ...
      {isAuthenticated && (
        <Stack spacing={4} onClick={logout}>
          <Flex
            py={2}
